...
for b in [2,3,4,5,6,10]:
    ...
    temp_PP = PreProcess.PreProcess(data,n_buckets=b,func='boolean')
    data_tmp = temp_PP.fit(data)
    
    for p in [1.01,1.1,1.5,2]:
        
        tmp = []  
        win = Winnow.Winnow(param=p)
...
filename = 'result/Winnow/weight_unbalanced_best_bucket_' + str(best_bucket) + '_best_param_' + str(best_param)
  
with open(filename, 'wb') as thefile:
    for item in unbalanced.W:
        thefile.write("%s\n" % item)
...
### Stat
'''  
filename = 'result/Winnow/stat_unbalanced_best_bucket_' + str(best_bucket) + '_best_param_' + str(best_param)
...
filename = 'result/Winnow/prediction_unbalanced_best_bucket_' + str(best_bucket) + '_best_param_' + str(best_param)
...
predictEval = unbalanced.predict(evalset)

##
predictEval = PP.processLabels(predictEval)
##
  
filename = 'result/Winnow/eval_unbalanced_best_bucket_' + str(best_bucket) + '_best_param_' + str(best_param) 
